...
draw_plot <- function(path1, path2){
  ...
  data_plot$tech <- factor(data_plot$tech, levels = c('DIG', 'LLL'))
...
rna <- draw_plot('../output/dig_rna_rogue.RDS',
                 '../output/lll_rna_rogue.RDS')+ggtitle('RNA')+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5))
adt <- draw_plot('../output/dig_adt_rogue.RDS',
                 '../output/lll_adt_rogue.RDS')+ggtitle('ADT')+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5))
atac <- draw_plot('../output/dig_atac_rogue.RDS',
                 '../output/lll_atac_rogue.RDS')+ggtitle('ATAC')+theme(plot.title = element_text(hjust = 0.5))
wnn <- draw_plot('../output/dig_wnn_rogue.RDS',
                 '../output/lll_wnn_rogue.RDS')+ggtitle('3WNN')+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5))
pdf('../plots/rogue.pdf', width = 16, height = 4)
wrap_plots(wnn, rna, adt, atac, ncol = 4)
dev.off()
...
adt <- draw_plot_new('../output/dig_adt_rogue.RDS',
                 '../output/lll_adt_rogue.RDS')+ggtitle('ADT')+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5))
atac <- draw_plot_new('../output/dig_atac_rogue.RDS',
                  '../output/lll_atac_rogue.RDS')+ggtitle('ATAC')+theme(plot.title = element_text(hjust = 0.5))
wnn <- draw_plot_new('../output/dig_wnn_rogue.RDS',
                 '../output/lll_wnn_rogue.RDS')+ggtitle('3WNN')+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5))
pdf('../plots/rogue_cluster.pdf', width = 16, height = 4)
wrap_plots(wnn, rna, adt, atac, ncol = 4)
dev.off()
