...
rna.markers <- FindAllMarkers(data, assay = 'ADT', min.pct = 0, logfc.threshold = 0)
rna.markers.1 <- FindMarkers(data, assay = 'ADT', ident.1 = 'ACT_IL1B_IL23', 'ACT_IL1B_IL23_PGE2', min.pct = 0, logfc.threshold = 0)
rna.markers.2 <- FindMarkers(data, assay = 'ADT', ident.1 = 'ACT_IL1B_IL23', 'ACT_IL1B_IL23_TGFB', min.pct = 0, logfc.threshold = 0)
rna.markers.3 <- FindMarkers(data, assay = 'ADT', ident.1 = 'ACT_IL1B_IL23', 'ACT_IL1B_IL23_PGE2_TGFB', min.pct = 0, logfc.threshold = 0)
rna.markers.4 <- FindMarkers(data, assay = 'ADT', ident.1 = 'ACT_IL1B_IL23_PGE2', 'ACT_IL1B_IL23_TGFB', min.pct = 0, logfc.threshold = 0)
rna.markers.5 <- FindMarkers(data, assay = 'ADT', ident.1 = 'ACT_IL1B_IL23_PGE2', 'ACT_IL1B_IL23_PGE2_TGFB', min.pct = 0, logfc.threshold = 0)
rna.markers.6 <- FindMarkers(data, assay = 'ADT', ident.1 = 'ACT_IL1B_IL23_TGFB', 'ACT_IL1B_IL23_PGE2_TGFB', min.pct = 0, logfc.threshold = 0)
...
save(DIG, LLL, file = '../output/ADT_markers.RData')
...
comparison_label <- function(index, name){
  ...
  tmp2$gene <- rownames(tmp2)
  tmp <- merge(tmp1, tmp2, by = 'gene', suffixes = c('_DIG', '_LLL'))
  tmp$adt <- str_split_fixed(str_split_fixed(tmp$gene, '-A0', 2)[,1], '-A1', 2)[,1]
  
  p1 <- ggplot(tmp, aes(x = avg_log2FC_LLL, y = avg_log2FC_DIG)) +
    geom_point() +
    geom_text_repel(max.overlaps = Inf, aes(label=ifelse(adt %in% c('CD127', 'CD62L', 'CD63', 'CD25', 'CD71', 'CD69', 'CD54'),adt,''), color=ifelse(avg_log2FC_DIG > 0, 'positive', 'negative')))+
    pretty_plot(fontsize = 10) + L_border() + 
    labs(x = "LLL log2FC", y = "DIG log2FC") +
