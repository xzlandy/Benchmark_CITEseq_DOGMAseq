...
save(DIG, CITE, file = '../output/RNA_markers_wt_intron.RData')

load('../output/RNA_markers_wt_intron.RData')
...
comparison <- function(index, name){
  ...
  cowplot::ggsave2(p1, file = paste0("../plots/log2FC_", name, "_wt_intron.pdf"), width = 4, height = 4)
  ...
  p1 <- ggplot(tmp, aes(x = avg_log2FC_LLL, y = avg_log2FC_DIG, color = exon_bi)) +
    ...
    theme(legend.position = "bottom", legend.direction = 'horizontal')
  cowplot::ggsave2(p1, file = paste0("../plots/log2FC_", name, "_bi_wt_intron.pdf"), width = 4, height = 4)
}
...
comparison_label <- function(index, name){
  ...
  p1 <- ggplot(tmp, aes(x = avg_log2FC_LLL, y = avg_log2FC_DIG, color = exon_bi)) +
    ...
    labs(x = "CITE log2FC", y = "DIG log2FC", color = "") +
    theme(legend.position = "bottom", legend.direction = 'horizontal')
  cowplot::ggsave2(p1, file = paste0("../plots/log2FC_", name, "_bi_label_wt_intron.pdf"), width = 4, height = 4)
}
