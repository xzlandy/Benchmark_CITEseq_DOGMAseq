...
load('../data/barcodes_wt_intron.RData')
...
save(DIG, CITE, file = "../output/complexity_matrices_wt_intron.rda")


if(FALSE){
  
  load("../output/complexity_matrices_wt_intron.rda")
  ...
  cowplot::ggsave2(p1, file = "../plots/tag_complexity_assay_MO_wt_intron.pdf", width = 1, height = 1.5)
...
if(FALSE){
  ...
  p1 <- ggplot(rbind(DIG, CITE), aes(x = assay, y = log10(complexity), color = condition)) +
    ...
    pretty_plot(fontsize = 7) + L_border() + scale_y_continuous(limits = c(2, 5)) +
    labs(x = "Condition", y = "log10 Tag Complexity", color = "")
  cowplot::ggsave2(p1, file = "../plots/tag_complexity_assay_MO_split_wt_intron.pdf", width = 3, height = 1.5)
  ...
  cowplot::ggsave2(p1, file = "../plots/tag_complexity_assay_MO_split_sample_wt_intron.pdf", width = 2, height = 2)
...
# <chr> <chr>                   <dbl> <int>
# 1 CITE  SB775372                 4216  7171
# 2 CITE  SB775393                 3942  6035
